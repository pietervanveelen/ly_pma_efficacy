---
title: "Propidium monoazide (PMA) effect on bacterial survival analysis: comparing PMA-qPCR, conventional qPCR and culture based estimates "
...
```{r install and load packages, warning=F, message=F, echo=F, eval=T, include=T}
...
qPCR_df_curve_corrected <- 
  ...
  mutate(log_copies_in_reaction_well = 
              case_when(primer == "bac_16S" ~ abs(ax)/abs(slope_16S),
                        ...
                        TRUE ~ NA_real_)) %>%
  mutate(copies_in_reaction_well  = 10^log_copies_in_reaction_well) %>%
  mutate(copies_mL_original_sample = copies_in_reaction_well/sample_volume_ul*1000) %>%
