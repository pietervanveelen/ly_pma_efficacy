...
data = openxlsx::read.xlsx("input_data/Exp1.1_PMA_data_qPCR.xlsx") %>% as_tibble()
data = 
...
data_exp1 = openxlsx::read.xlsx("input_data/Exp1.2_Cell_density_effect_Ecoli_cq_replaced.xlsx") %>% 
...
```
```{r check sampling within std curves}
...
```{r standard curves and check DL }

data_expII_lowconc = 
  read_csv2("input_data/raw_qPCR_data/E.coli_10^5_PMA_17-3-2023_NMID -  Quantification Cq Results.csv") %>% 
  janitor::clean_names() %>%
  mutate(exp_id = "E.coli_low") %>% 
  filter(!well %in% c("B11", "C02", "C05", "C06", "C10", "E05")) %>% 
  mutate(type = case_when(grepl("^Std", content) ~ "standard",
                           grepl("^Unk", content) ~ "sample",
                           grepl("NTC", content) ~ "blank_ntc",
                                 TRUE ~ NA_character_))

data_expII_midconc = 
  read_csv2("input_data/raw_qPCR_data/admin_2023-02-28 14-50-14_CT008763 -  Quantification Cq Results.csv", col_select = -1) %>% 
  janitor::clean_names() %>%
  mutate(exp_id = "E.coli_mid") %>% 
  filter(!well %in% c("D07", "D08", "D10", "D11", "D12", "E01", "E02")) %>% 
  mutate(type = case_when(grepl("^Std", content) ~ "standard",
                           grepl("^Unk", content) ~ "sample",
                           grepl("NTC", content) ~ "blank_ntc",
                                 TRUE ~ NA_character_))

data_replacement_lowconc = 
  read_csv2("input_data/raw_qPCR_data/NMID_22032023_NTC_controle -  Quantification Cq Results.csv", col_select = -1) %>% 
  janitor::clean_names() %>%
  mutate(exp_id = "E.coli_low") %>% 
  filter(grepl("Std|NTC", content) |  well %in% c(paste0("E0", seq(1:6)))) %>%
  filter(!well %in% c("A01", "A02")) %>% 
  mutate(type = case_when(grepl("^Std", content) ~ "standard",
                           grepl("^Unk", content) ~ "sample",
                           grepl("NTC", content) ~ "blank_ntc",
                                 TRUE ~ NA_character_))
...
data_replacement_midconc3.0= 
  read_csv("input_data/raw_qPCR_data/NMID_29032023_PMA_effluent_16s_ybbw -  Quantification Cq Results 3.0.csv", col_select = -1) %>% 
  janitor::clean_names() %>%
  mutate(exp_id = "E.coli_mid") %>% 
  filter(grepl("Std|NTC", content) |  well %in% c(paste0("G0", seq(1:7)))) %>%
  mutate(type = case_when(grepl("^Std", content) ~ "standard",
                           grepl("^Unk", content) ~ "sample",
                           grepl("NTC", content) ~ "blank_ntc",
                                 TRUE ~ NA_character_))

# Some wells had to be redone: Low starting quantity B11, C2, C5, C6, C10, E5; Medium starting quantity: D7, D8, D10, D11, D12, E1, E2
# data_redo_some_wells =
#   read_csv2("input_data/raw_qPCR_data/NMID_22032023_NTC_controle -  Quantification Cq Results.csv", col_select = -1) %>%
#   filter(grepl("Std|NTC", Content) |  Well %in% c(paste0("E0", seq(1:6)))) %>%
#     janitor::clean_names() %>%
#     mutate(type = case_when(grepl("^Std", content) ~ "standard",
...
#                                 TRUE ~ NA_character_)) %>% 
# bind_rows(
#   read_csv2("input_data/raw_qPCR_data/admin_2023-02-28 14-50-14_CT008763 -  Quantification Cq Results_cts_replaced.csv") %>%
#   filter(grepl("Std|NTC", Content) |  Well %in% c("D07", "D08", "D10", "D11", "D12", "E01", "E02")) %>%
#   janitor::clean_names() %>%
#   mutate(type = case_when(grepl("^Std", content) ~ "standard",
#                           grepl("^Unk", content) ~ "sample",
#                           grepl("NTC", content) ~ "blank_ntc",
...
data_expII_low_mid = 
  bind_rows(data_expII_lowconc %>% 
              mutate(exp_id = "E.coli_low") %>% 
              janitor::clean_names() %>% 
              mutate(cq = case_when(exp_id == "E.coli_low" & well == "E05" ~ data_replacement_lowconc %>% filter(well == "E06") %>% pull(cq), TRUE ~ cq)) %>% 
              mutate(starting_quantity_sq = case_when(exp_id == "E.coli_low" & well == "E05" ~ data_replacement_lowconc %>% filter(well == "E06") %>% pull(starting_quantity_sq), TRUE ~ starting_quantity_sq)) %>% 
              mutate(log_starting_quantity = log10(starting_quantity_sq)),
           data_expII_midconc %>% 
             mutate(exp_id = "E.coli_medium") %>% 
             janitor::clean_names() %>% 
             mutate(type = case_when(grepl("^Std", content) ~ "standard",
                                     grepl("^Unk", content) ~ "sample",
                                     grepl("NTC", content) ~ "blank_ntc",
...
std_low_plot = 
data_expII_lowconc %>%
  ggplot(aes(x = log_starting_quantity, y = cq)) +
  geom_smooth(data = filter(data_expII_lowconc, type == "standard"), method = "lm", se = F) +
  geom_point(aes(color = type, shape = type), size = 2, alpha = 0.7) +
  ggtitle("low_conc") +
  theme_light()
...
data_expII_midconc %>%
  ggplot(aes(x = log_starting_quantity, y = cq)) +
  geom_smooth(data = filter(data_expII_midconc, type == "standard"), method = "lm", se = F) +
  geom_point(aes(color = type, shape = type), size = 2, alpha = 0.7) +
  ggtitle("mid_conc") +
  theme_light()
...
data_replacement_lowconc %>% 
  ...
  geom_smooth(data = filter(data_expII_lowconc, type == "standard"), method = "lm", se = F) +
  geom_point(aes(color = type, shape = type), size = 2, alpha = 0.7) +
  ggtitle("low_conc_replac") +
  theme_light()

std_mid_repl_plot = 
data_replacement_midconc3.0 %>% 
  ggplot(aes(x = log_starting_quantity, y = cq)) +
  geom_smooth(data = filter(data_replacement_midconc3.0, type == "standard"), method = "lm", se = F) +
  geom_point(aes(color = type, shape = type), size = 2, alpha = 0.7) +
  ggtitle("mid_conc_replac") +
  theme_light()


patchwork::wrap_plots(std_low_plot, std_low_repl_plot, std_mid_plot, std_mid_repl_plot)

ggsave("figures/detection_limit_check_std_curves_expII.pdf", width = 7, height = 5)

# data_expII_low_mid %>% filter(is.na(type))
#   select(well,cq, starting_quantity_sq, log_starting_quantity, sq_mean, exp_id, type) %>% 
#   mutate(exp_id = as.character(exp_id)) %>% 
#   left_join(., 
#              data_exp1 %>% 
#                select(exp_id, well = "well_id", date, biol_rep, conc, pma, DNA_diluted, cq = "Ct", tech_duplo, copies_100ml, sq_calc) %>% 
#                mutate(letter = str_match(well, "^([A-H])")[,2],
#                       number = str_match(well, "^.[0-9]+$")[,1] %>% str_extract("[0-9]+") %>% str_pad(2, pad = "0"),
#                       well = paste0(letter, number)),
#              by = c("well", "exp_id")) %>% 
#   filter(!(type == "sample" & is.na(conc))) %>%
#   rename(cq = "cq.x") %>% 
#   ggplot(aes(x = log_starting_quantity, y = cq)) +
#   geom_smooth(method = "lm", se = F, color = "grey50", linewidth = 0.5) +
#   geom_point(aes(fill = factor(conc),color = factor(conc), shape = type), size = 2, alpha = 0.7) +
#   scale_shape_manual(values = c(21, 24, 22, 25)) +
#   facet_wrap(~exp_id + pma) +
#   theme_light()
...
```{r plot observed and predicted data starting concentrations, warning=F, message=F, fig.height=4, fig.width=6}
...
# save plot
ggsave(plot = plot_total_cells_NMID_DPOL2, filename = "figures/NMID_ExpII_total_cell_conc_effect_on_PMA_NMID_DPOL_updated.pdf", width = 8, height = 4)
...
```{r standard_curve statistics and plot ozone spiking experiment, warning=F, message=F}
